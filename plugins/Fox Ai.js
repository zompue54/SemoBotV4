...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Fox') && conf.TALKING_FOX !== 'true') {        
        ...
        var finm = message.message.replace('Fox', '').replace(' ', '')   
        ...
        await axios.get('http://api.brainshop.ai/get?bid=161443&key=o4TmUKcCYDFuU5KL&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            ...
            await message.client.sendMessage(message.jid,'*' + fins +'*', MessageType.text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(message.jid ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "*Fox AI*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 3000, "width": 3000, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./media/image/FOX.jpg')}}}});
        })
    }
}));
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_FOX == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Fox özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Fox yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Fox yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'Fox, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'Fox, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Fox features. Turn your account into a ai chatbot!'
    already_on = 'Fox artificial intelligence is already fully functional.'
    already_off = 'Fox artificial intelligence is currently running semi-functional.'
    succ_on = 'Fox Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'Fox Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Fox സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Fox കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Fox AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Fox പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Fox സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'fox ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.fox on / off' }, (async (message, match) => {
    var fox_status = `${conf.TALKING_FOX}`
    if (match[1] == 'on') {
        if (fox_status == 'true') {
